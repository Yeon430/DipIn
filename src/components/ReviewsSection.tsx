...
export default function ReviewsSection({ 
  ...
  useEffect(() => {
    fetchReviews()
  }, [missionId, page, sortBy, sortOrder])
  ...
  const fetchReviews = async () => {
    try {
      ...
      // 사용자의 도움됨 상태 확인
      if (userId) {
        const helpfulPromises = data.reviews.map((review: Review) => 
          fetch(`/api/reviews/${review.id}/helpful?userId=${userId}`)
            .then(res => res.json())
            .then(data => ({ ...review, ...data }))
            .catch(() => review)
        )

        const reviewsWithHelpful = await Promise.all(helpfulPromises)
        ...
        {reviews.map((review) => (
          <div key={review.id} className="border-b border-slate-200 pb-6 last:border-b-0">
            <div className="flex items-start gap-4">
              <img 
                src={review.user_profiles.avatar || '/default-avatar.png'} 
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-sm text-slate-500">
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-1 hover:text-slate-700 transition-colors ${
                      review.hasVoted && !review.isHelpful ? 'text-red-600' : ''
                    }`}
                  >
